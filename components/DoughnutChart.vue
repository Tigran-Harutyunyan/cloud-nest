<template>
  <div class="relative" :style="containerStyle">
    ...
    <div v-if="visibleValue" class="value-container">
      ...
      <template v-else>
        <h1 :class="classValue" :style="valueStyle" class="text-center">
...
export default {
  ...
  computed: {
    ...
    valueStyle() {
      let percentColor =
        this.foregroundColor && this.validateColor(this.foregroundColor)
          ? this.foregroundColor
          : "#1993ff";
      let percentSize =
        this.customPercentSize && this.customPercentSize < 60
          ? `${this.customPercentSize}px`
          : false;
      return {
        color: percentColor,
    ...
    customTextStyle() {
      let textColor =
        this.customTextColor && this.validateColor(this.customTextColor)
          ? this.customTextColor
          : this.foregroundColor;
      let textWidth = this.strokeWidth
        ? `calc(100% - ${this.strokeWidth * 2}px)`
        : "calc(100% - 20px)";
      let textPadding =
        this.strokeWidth && this.strokeWidth > 7 && this.strokeWidth <= 18
          ? `0 ${this.strokeWidth}px`
          : "0 10px";
      let textSize =
        this.customTextSize && this.customTextSize <= 22
          ? `${this.customTextSize}px`
          : false;
      let customTopMargin =
        this.strokeWidth && this.strokeWidth > 7 && this.strokeWidth <= 18
          ? this.strokeWidth
